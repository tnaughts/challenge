...
helpers do
  ...
  def release_tix_html # sets code on server side
    ...
    stories.sort_by { |s| s["current_state"] }.each do |story|
      ...
      story["owner_ids"].each do |o|
        owners += "#{User.find_by_poid(o).name} | "
      end
      unless owners.empty?
        owners
      else
  ...
  def update_users
    ENV['PT_PROJECTS'].split(", ").each do |id|
      ...
      ownersDatum.each do |ownerData|
        unless User.find_by_poid(ownerData["person"]["id"])
          User.create( poid: ownerData["person"]["id"],
